...
JS;
    ...
    return form::checkbox($this->name($model),null,$s,['disabled'=>'disabled','class'=>'non-form-control']);
}
...
function _item($model){
	...
	return form::checkbox($this->name($model),null,$s,$params);
...
function handler_save($data,$old_data,$model){
    ...
    $can_edit = ($this->param['can_edit']??true);
    if(!$can_edit) {
        return $model;
    }
    ...
    if($r==1) {
        ...
        if($use_time) {
		if(isset($old_data[$this->name]) && $old_data[$this->name]>0) {
                return $model;
            }
            $r=time();
        }
