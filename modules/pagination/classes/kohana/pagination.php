...
class Kohana_Pagination {
	...
	public function geturl($page=1){
		$a=array_merge($this->_route_params,array($this->config['current_page']['key'] => $page));
		$a['controller']=$this->_request->controller();
		$a['action']=$this->_request->action();
		return $this->_route->uri($a);
	...
	public function url($page = 1)
	{
		...
		switch ($this->config['current_page']['source'])
		{
			case 'query_string':
				...
				return URL::site($this->geturl($page).$this->query(array($this->config['current_page']['key'] => $page)));

			case 'route':

				return URL::site($this->geturl($page)).$this->query();
		}
