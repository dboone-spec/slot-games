...
class game_slot_novomatic_bookofrad extends game_slot_novomatic_deluxe
{
    ...
    public function restore()
    {
        ...
        if($this->_session['freeCountAll']>0 && $this->_session['freeCountCurrent']==0) {
            $ans['bonus']=$this->_session['freeCountAll']-1;
        }
        
        if($this->_state()==3) {
            $ans['extra_param'] = $this->_session['extra_param'];
        }
        if($ans['bonus_all']>0 && $ans['bonus_all']==$ans['bonus_win']) {
            return $ans;
        }
        if($this->_state()==1) {
            $ans['bonus_all']=0;
    ...
    public function bonus_game() {
        ...
        if($this->_calc->bonus_win>0) {
            $ans['bonus_super_symbol_win']['win']=$this->_calc->bonus_win*100;
