...
class Controller_Admin1_Statisticgamemonth extends Controller_Admin1_Base
{
...
public function action_index()
{
	...
	$currency=arr::get($_GET, 'currency',false);
    if($currency!==false) {
    ...
    $sql_games ="Select g.visible_name as game,
            CASE
                ...
                ELSE
                    'normal'
            END AS bettype,
             sum(count) as count, sum(amount_in*$val) as in, sum(amount_out*$val) as out,
                sum(amount_in*$val-amount_out*$val) as win
        From statistics s
	...
	if($currency!==false) {
        $sql_games .= ' and o.currency_id=:currency_id';
        $sqlOffice.=' and o.currency_id=:currency_id';
    }
    ...
    $statistic = db::query(1, $sql_games)->parameters([
        ...
        ':partner'=>$partnerId,
        ':isTest'=>$is_test,
		':currency_id'=>$currency?$currency->id:false,
    ...
    if(Person::$user_id==1214) {
        $partnersList+=Person::user()->sergeiAccOwners;
    }
    else {
    ...
    $view=new View('admin1/statisticgame/month');
    $view->month   = $month;
    $view->year     = $year;
