...
class Controller_Admin1_Reportmonth extends Controller_Admin1_Base
{
    ...
    public function action_index()
    {
        ...
        $currenciesChoosed = arr::get($_GET,'curr',[]);
        $allCurrencies=[];
        ...
        foreach ($statistic as &$row) {
            ...
            if(!empty($currenciesChoosed) && !in_array($currency,$currenciesChoosed)) {
                continue;
            }
        ...
        if(empty($currenciesChoosed)) {
            $currenciesChoosed=$allCurrencies;
        }
        ...
        if (arr::get($_GET, 'xls') == 'go') {
			...
			$data[] = [''];
            $data[] = [''];
            $data[] = ['6) If currency not exists at xe.com, find out it at https://coinmarketcap.com/, for example TRX'];
            $data[] = ['7) Choose "EUR" currency at settings'];
            $data[] = ['8) Press the key combination Ctrl+F'];
            $data[] = ['9) In the window, find "See historical data"'];
            $data[] = ['10) Click "See historical data" and find DATE '.$timeTo];
            $data[] = ['11) Use "Close**" column'];
        ...
        $view->months = $months;
        $view->years = $years;
        $view->currencies = $allCurrencies;
        $view->currenciesChoosed = $currenciesChoosed;
