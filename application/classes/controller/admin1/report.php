...
class Controller_Admin1_Report extends Controller_Admin1_Base
{
    ...
    protected function getCurrencyDay($day)
    {
        list($y, $m, $d) = explode('-', $day);
        $time = mktime(0, 0, 0, $m, $d, $y);
        //курс за первое число является курсом за предыдущий месяц
        return strtotime('last day of this month', $time) + 60 * 60 * 24;
    ...
    public function action_index()
    {
        ...
        $convert = arr::get($_GET, 'convert', 0);
        ...
        list($y, $m, $d) = explode('-', $time_from);
        $timeFromInt = mktime(0, 0, 0, $m, $d, $y);

        list($y, $m, $d) = explode('-', $time_to);
        $timeToInt = mktime(0, 0, 0, $m, $d, $y);
        ...
        $sql_users = "select s.date||'::'||s.office_id as id,s.users,s.newusers
        ...
        $sql_games .= ' GROUP BY date, s.office_id, c.code, c.mult, c.val
        ...
        $total = ['in' => 0,
            ...
            'jp' => 0, 'promoout' => 0, 'promocnt' => 0, 'users' => 0,'owners'=>[]];


        $currenciesValues = [];
        $days = [];
        if ($convert) {
            for ($day = $timeFromInt; $day <= $timeToInt; $day += 60 * 60 * 24) {
                //курс за первое число является курсом за предыдущий месяц
                $v = strtotime('last day of this month', $day) + 60 * 60 * 24;
                $days[$v] = $v;
            }

            $sql = 'select date, currency, value
                    from currency_rates
                    where date in  :days ';

            $currenciesData = db::query(1, $sql)->param(':days', $days)->execute()->as_array();
            $currenciesValues = [];
            foreach ($currenciesData as $row) {
                $currenciesValues[$row['date']][$row['currency']] = $row['value'];
            }
            ...
            foreach ($statistic as $key => $row) {
                //$c= date,c.code,
                $c = 1;
                $day = $this->getCurrencyDay($row['date']);
                $c = $currenciesValues[$day][$row['code']] ?? false;

                if ($c === false) {
                    $day = 0;
                    $c = 0;
                    //следующий месяц еще не начался и можно взять текущие курсы
                    list($y, $m, $d) = explode('-', $row['date']);
                    $nextDate = mktime(0, 0, 0, $m, $d, $y);
                    $nextDate = strtotime('last day of this month', $nextDate) + 60 * 60 * 24;
                ...
                $statistic[$key]['in'] = $row['in'] * $c;
                $statistic[$key]['out'] = $row['out'] * $c;
                $statistic[$key]['win'] = $row['win'] * $c;
                $statistic[$key]['cfsin'] = $row['cfsin'] * $c;
                $statistic[$key]['lfsin'] = $row['lfsin'] * $c;
                $statistic[$key]['afsin'] = $row['afsin'] * $c;
                $statistic[$key]['cfsout'] = $row['cfsout'] * $c;
                $statistic[$key]['lfsout'] = $row['lfsout'] * $c;
                $statistic[$key]['afsout'] = $row['afsout'] * $c;
                $statistic[$key]['jp'] = $row['jp'] * $c;
                $statistic[$key]['promoout'] = $row['promoout'] * $c;
                //в базе курс на 1 число месяце, хранит данные за предыдущее число
                $statistic[$key]['currencyDay'] = $day > 0 ? date('Y-m-d', $day - 60 * 60 * 24) : '';
                $statistic[$key]['currencyValue'] = $c;
            }
        ...
        foreach ($owner_offices_sql as $s) {
            $owner_offices[$s['id']] = arr::get($owners_sql, $s['owner'], ['comment' => ''])['comment']; //todo uncomment
        }
        ...
        foreach ($statistic as $row) {
            ...
            $total['offices'][$row['office_id']]['owner'] = $owner_offices[$row['office_id']] ?? 'anonowner';
            ...
            if ($only_total && $convert) {
                if (isset($total['owners'][$owner_offices[$row['office_id'] ?? 'anonowner']])) {
                    $total['owners'][$owner_offices[$row['office_id']]]['in'] += $row['in'];
                    $total['owners'][$owner_offices[$row['office_id']]]['mult'] = $row['mult'];
                    $total['owners'][$owner_offices[$row['office_id']]]['cfsin'] += $row['cfsin'];
                    $total['owners'][$owner_offices[$row['office_id']]]['lfsin'] += $row['lfsin'];
                    $total['owners'][$owner_offices[$row['office_id']]]['afsin'] += $row['afsin'];
                    $total['owners'][$owner_offices[$row['office_id']]]['cfsout'] += $row['cfsout'];
                    $total['owners'][$owner_offices[$row['office_id']]]['lfsout'] += $row['lfsout'];
                    $total['owners'][$owner_offices[$row['office_id']]]['afsout'] += $row['afsout'];
                    $total['owners'][$owner_offices[$row['office_id']]]['out'] += $row['out'];
                    $total['owners'][$owner_offices[$row['office_id']]]['win'] += $row['win'];
                    $total['owners'][$owner_offices[$row['office_id']]]['count'] += $row['count'];
                    $total['owners'][$owner_offices[$row['office_id']]]['newusers'] += $statisticUsers["{$row['date']}::{$row['office_id']}"]['newusers'] ?? '0';
                    $total['owners'][$owner_offices[$row['office_id']]]['jp'] += $row['jp'];
                    $total['owners'][$owner_offices[$row['office_id']]]['promoout'] += $row['promoout'];
                    $total['owners'][$owner_offices[$row['office_id']]]['promocnt'] += $row['promocnt'];
                    $total['owners'][$owner_offices[$row['office_id']]]['users'] += $statisticUsers["{$row['date']}::{$row['office_id']}"]['users'] ?? '0';


                } else {
                    $total['owners'][$owner_offices[$row['office_id'] ?? 'anonowner']] = ['in'       => $row['in'],
                                                                                          'mult'     => $row['mult'],
                                                                                          'cfsin'    => $row['cfsin'],
                                                                                          'lfsin'    => $row['lfsin'],
                                                                                          'afsin'    => $row['afsin'],
        ...
        foreach ($total['owners'] as $key => $val) {
            $total['owners'][$key]['rtp'] = $total['owners'][$key]['in'] == 0 ? '&nbsp;' : round($total['owners'][$key]['out'] / $total['owners'][$key]['in'] * 100, 2);
        }
        ...
        $view->data = $data;
        $view->currenciesValues = $currenciesValues;
        $view->total = $total;
