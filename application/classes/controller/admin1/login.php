...
class Controller_Admin1_Login extends Controller_Template {
...
public function action_index() {
    ...
    if(isset($_SERVER['HTTP_HOST'])){
        if ((strpos($_SERVER['HTTP_HOST'],'agt') === false)){
            $logo = false;
        }
    }

    $view = View::factory('/admin1/login/index');
    $view->logo = $logo;
    $view->dir = $this->dir;
    $view->error=false;
...
public function action_login(){
    ...
    if(!th::officeIsEnable($person->office_id)) {
        $ans['error']=__('You entered an invalid username or password! Try again!');
        $this->response->body(json_encode($ans));
        return null;
    }
    
    if($person->role=='sa' and !th::flag('saLogin')) {
        $ans['error']=__('You entered an invalid username or password! Try again!');
        $this->response->body(json_encode($ans));
        return null;
    }
    
    if($person->role!='sa' and !th::flag('personLogin')) {
        $ans['error']=__('You entered an invalid username or password! Try again!');
        $this->response->body(json_encode($ans));
        return null;
    }
    ...
    if ($person->enable_telegram!=0 and empty($code)){
        ...
        if (empty($person->tgname) or empty($person->tgchatid) ){
            ...
            $tryfind=(new Model_Person())
                    ->where('tgname','=',$telegram)
                    ->where('tgchatid','is not',null)
                    ->limit(1)->find();

            if($tryfind->loaded()) {
                $person->tgchatid=$tryfind->tgchatid;
            }
