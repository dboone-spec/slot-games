...
class Controller_Admin1_Counterspartners extends Controller_Admin1_Base
{
...
public function action_index()
{
    ...
    $sql='select
			...
			sum(c.fs_lucky_in) as fs_lucky_in, sum(c.fs_lucky_out) as fs_lucky_out, sum(c.fs_lucky_count) as fs_lucky_count,
            sum(c.fs_lucky_in - c.fs_lucky_out) as fs_lucky_win

            from counters_partners c
    ...
    $total=[
			...
			'fs_lucky_in'=>0, 'fs_lucky_out'=>0, 'fs_lucky_win'=>0,'fs_lucky_count'=>0, 'fs_lucky_rtp'=>0,
    ...
    foreach($data as &$row){
        foreach($total as $k=>$v) {
            ...
            elseif($k=='fs_cash_rtp') {
			 ...
			 elseif($k=='fs_lucky_rtp') {
                if($total['fs_lucky_in']>0) {
                    $total[$k]=(round($total['fs_lucky_out']/$total['fs_lucky_in'],2)*100);
                }
            }
            else {
                $total[$k]+=$row[$k];
    ...
    $view=new View('admin1/counters/index');
