...
class Controller_Admin1_Person extends Super
{
    ...
    public function configure()
    {
        ...
        $this->show = ['name', 'fio', 'amount', 'password', 'enable_telegram', 'role', 'office_id'];
        ...
        if (Person::$role == 'sa') {
			...
			$this->show[] = 'our_api';
            $this->list[] = 'our_api';
            $this->vidgets['our_api'] = new Vidget_CheckBox('our_api', $this->model);
            $this->vidgets['owner_id'] = new Vidget_Integer('owner_id', $this->model);
        }
        ...
        $this->vidgets['office_id'] = new Vidget_Personoffice('office_id', $this->model);
        $officesList = Person::user()->officesName(null, true);
        $this->vidgets['office_id']->param('list', $officesList);
    ...
    public function canItem()
    {
        ...
        if ($this->model->loaded()) {
            ...
            $this->vidgets['role']->param('can_edit', false);
            if ($this->model->role == 'cashier' && !empty($this->model->office_id)) {
    ...
    public function handler_save($data)
    {
        ...
        if (person::$role == 'sa' && $this->model->loaded()) {
        ...
        if (Person::$role == 'gameman') {
            ...
            if ($this->model->loaded()) {
                $data['role'] = $this->model->role;
            } elseif (!in_array($data['role'], ['client', 'gameman'])) {
                $data['role'] = 'client';
            }
