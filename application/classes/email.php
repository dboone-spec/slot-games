...
class Email {
	...
	public static function connect($config = NULL)
	{
		...
		switch ($config['driver'])
		{
			...
			case 'sendmail':
				...
				$transport = new Swift_SendmailTransport(empty($config['options']) ? "/usr/sbin/sendmail -bs" : $config['options']);
		...
		return Email::$mail = new Swift_Mailer($transport);
	}
	 ...
	 */
	public static function send($to, $from, $subject, $message, $html = FALSE,$attach=null,$attachname=null)
	{
		...
		// Create the message
		$message = new Swift_Message($subject, $message, $html, 'utf-8');
		...
		if(!empty($attach)) {
            $message->attach(
                (new Swift_Attachment($attach))
                    ->setFilename($attachname.'.csv')
            );
        }
        ...
        /*
         * Устанвливает конфигурацию для отправки сообщений
         * 
         * @param   string        name of configuration
         */
        public static function config($name=NULL){  
            
            self::$config = Kohana::$config->load('email'); 
            
            if($name){
                
                self::$config = Kohana::$config->load("mail.$name"); 
        ...
        public static function from(){
            ...
            if(self::$config === NULL){
                return Kohana::$config->load('email.options.username');
            }    
            
            return self::$config['options']['username'];
