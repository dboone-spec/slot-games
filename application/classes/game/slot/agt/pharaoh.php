...
class game_slot_agt_pharaoh extends game_slot_agt
{
    ...
    public function bonus_game()
    {
        ...
        foreach($ans['bonus_super_symbol_win']['linesValue'] as &$lv) {
            $lv=rtrim(sprintf('%.'.$mult.'F',$lv),'0');
        }

        $ans['bonus_super_symbol_win']['win']=rtrim(sprintf('%.'.$mult.'F',$ans['bonus_super_symbol_win']['win']),'0');
        ...
        $ans['session_total_win_free'] = rtrim(sprintf('%.'.$mult.'F',$this->_session['total_win_free']),'0');

		$ans['session_total_win_free'] = rtrim(sprintf('%.'.$mult.'F',$this->_calc->total_win_free),'0');
