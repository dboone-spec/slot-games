...
class Api_Infin extends gameapi{
    ...
    public function checkOffice($currency,$partner,$test=1) {
        ...
        if (!$currency->loaded()  || $currency->disable!=0 ){
            throw new Exception("Cann't create offiice with currency $currency ");
        }
...
SQL;
    ...
    public function getGame($user_id,$lang,$force_mobile=false,$no_close=true,$closeurl=false,$cashier_url=false) {
        $domain = kohana::$config->load('static.gameapi_domen_infin');
    ...
    public function send($data,$extra_log=[]){
        ...
        logfile::create(date('Y-m-d H:i:s')." request: $extra_log_str $url \r\n $data",'infin');
        ...
        logfile::create(date('Y-m-d H:i:s')." response [$response_time]: $extra_log_str $ans",'infin');
...
XMLL
    ...
    public function bet($login,$url, $params=[],$is_repeat=false) {
        ...
        elseif(in_array($params['game'],['tothemoon'])) {
            $fin=0;
        }
        ...
        if($params['game']!='jp' && $params['game']!=$u->last_play_game) {
...
XMLL;
    ...
    public function processWrongBets($user_id) {
        ...
        if($this->gameName!='tothemoon' && count($bets)>3) {
                    ...
                    try {
                        ...
                        if($result) {
                            try
                            {
                                ...
                                if($bet->game=='tothemoon' && !empty($bet->initial_id)) {
                                    $info.='; '.$bet->initial_id;
                                }
                                $betArr['info']=$info;
                                ...
                                if($betArr['game']=='tothemoon' && $betArr['initial_id']>0) {
