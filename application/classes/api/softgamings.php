...
class Api_SoftGamings extends gameapi
{
    ...
    public function createAnonym($userId,$office_id){
    ...
    public function checkOffice($currency,$partner,$casino)
    {
		...
		$external_name='sg'.$currency->id.$partner.$casino;
    ...
    public function createOffice($currency,$partner,$casino)
    {
		...
		if(in_array((int) $casino,[6140,6156,5366,6313,6498,6874,6754,5482,5486,5530,6754,6770,6734,6934,6806,6922,7062,7194,6902,6942,7406,7414,7518])) {
            $o->max_win_eur=100000;
        }
		
		if(in_array((int) $casino,[1051]) && $currency->code=='VES') {
            $o->bet_min=1;
        }
		
		if(in_array((int) $casino,[901]) && $currency->code=='XOF') {
            $o->bet_min=25;
        }
		
		if(in_array((int) $casino,[901]) && $currency->code=='TZS') {
            $o->bet_min=200;
        }
        ...
        try {
            ...
            $sql_games = <<<SQL
                ...
                From games g
                Where g.provider = 'our' and brand ='agt' and show=1 and softg_show=1 and g.category!='coming' 
                and g.branded=0
SQL;
    ...
    protected function _send_win_request($url,$data,$office) {
        logfile::create(date('Y-m-d H:i:s').' ['.$office->id.']['.$data['roundID'].'] request win: '
            .$url.'win'.PHP_EOL.json_encode($data,1),'softgamings');
...
//        $data['debugCode']=3000;
    ...
    protected function _send_cancel_request($url,$data,$office) {
        ...
        //betwin
        if(isset($data['betAmount'])) {
            $data['amount']=$data['betAmount'];
            unset($data['betAmount']);
        }
        if(isset($data['betFraction'])) {
            $data['fraction']=$data['betFraction'];
            unset($data['betFraction']);
        }

        unset($data['roundEnded']);
        unset($data['winAmount']);
        unset($data['winFraction']);
        unset($data['winID']);
    ...
    public function bet($login,$url, $params=[],$is_repeat=false) {
        ...
        $amount = "".floor(bcmul($params['amount'],pow(10,$mult)));
        $win = "".floor(bcmul($params['win'],pow(10,$mult)));
        ...
        if($need_send_betrequest && $params['amount']==0 && $params['win']==0 && $params['bettype']!='free' && !isset($params['freeroundID'])) {
            return true;
        }
        ...
        if(($params['is_freespin'] && !isset($params['freeroundID'])) || $params['bettype']=='jp') {
            $need_send_betrequest=false;
            $noBetWin=true;
        }
		...
		if(in_array($u->office->partner,[6818,5984,6902,7406])) {
            $url='https://apac-1-api-4.fundist.org/System/Merchants/site-domain/Callback';
        }
        ...
        //win
			...
			if($noBetWin) {
                $data["game"]=($params['game'] == 'jp') ? bet::getJP()->game : $params['game'];
            }
        ...
        if($request_type=='bet' || $request_type=='betwin') {
            ...
            if ($new_balance === false) {
                if($request_type=='betwin' && $this->last_error_code==2011) {
                    ...
                    $can_cancel=false;
                    if($request_type=='bet' && $this->last_error_code==1001) {
                        $can_cancel=true;
                    }
                    if($request_type=='betwin' && ($this->last_error_code==1004 || $this->last_error_code==2000)) {
                        $can_cancel=true;
                    }
...
//                    return false;
	...
	public static function checkAndActivateFS(Model_User $u,Model_Game $game,Model_Freespin $fs) {

        $betLevel=(int) $fs->amount;

        $fs->lines=Kohana::$config->load('agt/' . $game->name.'.lines_choose')[0];

        $bets=$game->getAllBets($u->office,$u->office->currency,$fs->lines);

        $fs->amount=$bets[$betLevel]??$bets[0];

        if($betLevel>=count($bets)) {
            $fs->amount=max($bets);
        }

        $update = "update freespins set active=1,amount={$fs->amount},lines={$fs->lines} where id={$fs->id} and active=0 returning id";
        $r = Database::instance()->direct_query($update);
        if(count($r)) {
            $id = $r[0]['id'];
            $expirtime=time()+20*60;
            Database::instance()->direct_query("update freespins_history set active=1,amount={$fs->amount},lines={$fs->lines},expirtime=$expirtime where freespin_id={$id} and active=0");
        ...
        if(count($bets)) {
            ...
            foreach($bets as $bet) {
                ...
                //win
                    ...
                    $data["noBetWin"]=(int) ($bet->is_freespin>0 || $bet->game=='jp');
